...
* 模组可以控制item的隐私权; 隐私权, 具体可以分为
  * 公有(**public**): 某个item可以被外部代码使用, 
  * 私有(**private**): 某个item不能被外部代码使用, 只是内部的实现
...
* 在模组中可以存放item, 如结构, 枚举, 常数, 特征, 函数甚至其他的模组
* 也可以将对应的定义放在一起
...
* 如果想要访问模组中的item, 必须保证路径中的模组以及该item是可访问的
...
* 注意: 路径上的item有些是无法访问的, 因此编译会报错, 下小节将修复错误
...
* 在模组中的所有item默认是私有的;
  * 在父模组中的item不能使用子模组的私有item
  * 子模组的item可以使用父模组的item, 以及更上层的祖模组(**ancestor module**)
* `pub`关键字可以使item变为公有的
...
`use`关键字可以将一条路径引入到一个作用域中; 当引入路径后, 可以像局部item那样使用该路径上的item
...
用`use`引入结构, 枚举类和其他item时, 惯用做法是引入该item

* 并没有什么原因要这样做; 只是习惯做法而已
* 注意: 当要引入两个相同名字的item时, 不能这么做
...
当需要使用某个包中或某个模组中的多个item, 可以使用嵌套路径
...
* 例子引入了`std::collection`中的所有公有item

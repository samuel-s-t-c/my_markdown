...
* 函数重载, 函数匹配
* 函数类型, 函数指针
* 默认实参, `inline`函数, `constexpr`函数, `assert`宏和`NDEBUG`
...
* 局部变量的名字隐藏它在外层作用域中的声明; 
  * 隐藏, 是指该名字指向了另一个实体, 与在外层作用域所声明的实体不同的实体
* 局部变量的寿命取决于它如何定义的
...
* 作用: 在函数调用时可以省略有默认实参的形参; 这样做的话该形参被默认实参初始化

* 用法: 在函数声明或定义时, 把默认实参作为形参的初始符出现在形参列表中
...
* 例子: 
  ...
  //形参名可省略
  string screen(sz ht = 32, sz wid = 80, char backgrnd = ' '); 
  ```
...
* 如果打算使用默认实参, 在调用函数时省略对应位置的实参即可, 它后面的所有实参也必须省略.
...
一个函数可以声明多次; 但是, 在给定的作用域中一个形参只能被赋予一次默认实参

* 也就是说, 在一个作用域中, 函数的后续声明只能为之前没有默认实参的形参添加默认实参

```c++
//假如有如下声明
string screen(sz, sz, char = ' ');
//在同一作用域中不允许给已有默认实参的形参赋予默认值
string screen(sz, sz, char = '&');		//error
{
//在新的作用域中并可以为第三个形参赋予默认值
string screen(sz, sz, char = '&');		//ok
//在同一作用域中不允许给已有默认实参的形参赋予默认值
string screen(sz, sz, char = '*'); 	    //error
//在同一作用域中允许为其他没有默认实参的形参赋予默认值
...
* 局部变量不能作为默认实参
* 除此之外, 只要表达式的类型能够转换成形参所需的类型, 该表达式就能作为默认形参

在编译时, 先进行名字解析, 再进行类型检查; 在运行时, 对名字求值

* 名字解析时, 用作默认实参的名字在函数所在的作用域内解析
...
* 对名字求值的过程发生在函数调用时
  ...
  void f2()
  {
      ...
      sz wd = 100;		//does not changes the default argument
...
* 有些那种规模较小的操作, 它既可以定义成函数也可以写成表达式; 

* 把这种操作定义成普通函数的话, 利用函数的性质, 使用起来很便利, 但是调用一个函数一般比求值等值表达式慢一些.

内联函数和`constexpr`函数通常定义在头文件中

### 内联函数

* 动作: 如果可能, 内联函数在每个调用点上行内展开

* 用法: 在函数定义时将`inline`放在返回类型前

  ```c++
  inline type name(parameter_list) {/*...*/};
  ```

* 说明:
  * 一般来说, `inline`用于优化规模较小, 简单和经常调用的函数
    * 大于75行的函数不怎么可能在行内展开
    * 很多编译器不支持内联一个递归函数
  * `inline`的说明只是向编译器发出的请求, 编译器可以选择忽略该请求
  * `inline`函数可以在程序中多次定义; 因为编译器需要函数定义来展开函数

### `constexpr`函数(C++11)

`constexpr`函数

* 作用: 编译器会检验`constexpr`函数是否返回常量表达式;

* 用法: 在函数定义时把`constexpr`放在返回类型前

  ```c++
  constexpr type name(parameter_list){/*...*/}
  ```
  * 返回类型和形参类型都必须是字面值类型
  * 函数体内必须包含一条`return`语句
  * 函数体内可以包含在运行时不会执行任何操作的语句, 如空语句, 类型别名语句, `using`声明等

* 动作: 在需要`constexpr`类型的地方使用`constexpr`函数

  * 如果`constexpr`函数返回常量表达式, 编译器把函数调用替换成其结果值
  * 如果该函数不是返回常量表达式, 编译器将发出错误信息

* 说明:

  * `constexpr`函数不一定返回常量表达式
  * `constexpr`函数被隐式地定义为内联函数, 为了能在编译过程中随时展开
  * `constexpr`函数可以在程序中多次定义; 因为编译器需要函数定义来展开函数

* 例子

  ```c++
  constexpr int new_sz() {return 42;}
  constexpr int foo = new_sz();			//new_sz是constexpr函数
  //只有cnt的实参是常量表达式时, scale才返回常量表达式
...
* 当应用程序编写完成并准备发布时, 可以屏蔽掉调试代码

两种预处理器的功能: `assert`和`NDEBUG`

### `assert`预处理宏

预处理宏(**preprocessor marco**), 是一个预处理器变量, 它的行为与内联函数相似.

`assert`预处理宏: 定义在`cassert`头文件

* 用法: `assert`接受一个表达式作为条件
  *  `assert(expr);`
* 具体动作: 
  * 首先对`expr`求值
  * 如果结果为`false`, `assert`输出信息并终止程序
  * 如果结果为`true`, `assert`什么都不做
* 说明:
  * `assert`是一个预处理器变量; 预处理名字由预处理器管理

### `NDEBUG`预处理器变量

* 定义`NDEBUG`: 
  * 在程序中定义`NDEBUG`: `#define NDEBUG`
  * 很多编译器提供一个命令行选项使我们可以定义预处理器
    * GCC中的`-D`, 微软c++编译器中的`/D`
* 与`assert`的关系
  * 如果定义了`NDEBUG`, 则`assert`什么都不做
  * 如果没有定义`NDEBUG`, 也就是默认状态下, `assert`将执行运行时检查
...
* `__func__`: 用于存放函数名的`const char`数组, 是`local static`的
* `__FILE__`: 文件名的字符串字面值
* `__LINE__`: 当前行号的整型字面值
...
函数匹配的第一步是选定本次调用对应的所有候选函数; 

候选函数(**candidate functions**), 是指满足以下条件的函数
...
函数匹配的第二步是从候选函数中选出可行函数

* 可行函数, 是指满足以下条件的函数
  * 其实参数量与形参数量相同
  * 每个实参与对应的形参匹配: 类型相同, 或者能转换成形参的类型

* 如果没有可行函数,编译器将报告无匹配函数的错误信息

### 找出最佳匹配(如果有的话)

函数匹配的第三步是从可行函数中选出一个最佳匹配的函数

* 确定最佳匹配的条件
  * 该函数中每个实参的匹配都不劣于其他可行函数的匹配
  * 至少有一个实参的匹配优于其他可行函数的匹配

* 如果没有最佳匹配, 编译器将报告二义性调用的错误信息

## 6.1实参匹配的等级*

为了确定最佳匹配, 编译器将实参类型与形参类型的匹配划分成几个等级; 

1. 精确匹配, 包括以下情况
   * 实参类型和形参类型相同
   * 实参从数组类型或函数类于转换成对应的指针类型
   * 添加或删除实参的顶层`const`
2. 需要`const`转换的匹配
3. 需要类型提升的匹配
4. 需要算术类型转换或指针转换的匹配
...
* 所有算术类型转换的级别都一样;
  ...
  * 字面值`3.14`的类型是`double`; 它既能转换成`long`也能转换成`float`, 而这两种转换的级别一样, 所以该调用具有二义性

# 7.函数指针

函数的类型由返回类型和形参列表组成的, 不包括函数名. 例子:

```c++
bool lengthCompare(const string&, const string&);
```

* `lengthCompare`函数的类型是`bool (const string&, const string&)`

一个函数指针, 就是一个指向函数的指针; 例子:

```c++
bool (*pf)(const string&, const string&);	//uninitialized
```

* `pf`是一个指向函数的指针, 它指向`bool (const string&, const string&)`类型的函数

* 包围`*pf`的圆括号是必要的

### 使用函数指针

当我们把函数名作为一个值使用时, 该函数自动转换成一个指向它本身的指针.

```c++
pf = lengthCompare;		//函数转换成指向它本身的指针
pf = &lengthCompare;	//对函数取地址得到一个指向它本身的指针
```
...
bool b2 = (*pf)("hello", "goodbye");//解引用pf, 得到lengthCompare函数
```

指向不同函数类型的指针之间没有转换规则

可以为函数指针赋一个`nullptr`或值为0的常量表达式, 表示该指针没有指向任何一个函数

### 函数指针作为形参

形参不能定义为函数类型, 但可以将形参定义为指向函数的指针

* 把形参定义为函数类型时, 编译器自动将函数类型转换成对应的函数指针类型
...
* 可以使用类型别名简化: `typedef`, `decltype`, `using`
  * `decltype`的表达式为函数名时, 返回函数类型, 而不是函数指针类型

### 返回函数指针

不能返回一个函数类型, 但可以返回一个函数指针

返回类型必须写成指针形式; 作为返回类型时, 编译器不会自动将函数类型转换成对象的函数指针类型
